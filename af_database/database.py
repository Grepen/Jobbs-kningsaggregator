...
class DBConnectionHandler:
    ...
    def create_db_table(self):
        ...
        self.cursor.execute(f"CREATE TABLE {DB_TABLE_NAME} (company TEXT, email TEXT, workplace TEXT, occupation TEXT, city TEXT, headline TEXT, published TEXT, deadline TEXT, af_url TEXt, id TEXT)") # added cmpny
    ...
    def get_fields_from_ad(self, ad):
        ...
        workplace = ad.get('employer', {}).get('workplace', ' ')
        ...
        return company, email, workplace, occupation, city, headline, published, deadline, af_url, ad_id

    def insert_one_ad(self, ad):
        company, email, workplace, occupation, city, headline, published, deadline, af_url, ad_id = self.get_fields_from_ad(ad)
        ...
        self.execute_sql(sql_query, payload=(company, email, workplace, occupation, city, headline, published, deadline, af_url, ad_id), ad_id=ad_id)
    ...
    def update_ad(self, ad):
        ...
        sql_query = f"UPDATE {DB_TABLE_NAME} SET company=?, email=?, workplace=?, occupation=?, city=?, headline=?, published=?, deadline=?, af_url=? WHERE id=?"
        self.execute_sql(sql_query, payload=(company, email, workplace, occupation, city, headline, published, deadline, af_url, ad_id), ad_id=ad_id)
